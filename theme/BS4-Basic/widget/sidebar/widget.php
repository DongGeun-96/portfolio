...
<aside id="nt_sidebar" class="bg-light font-weight-normal">
	...
	<!-- Top Head -->
	<div id="nt_sidebar_header" class="sidebar-head na-shadow px-3 mb-0">
		<h3 class="clearfix f-mo font-weight-bold en">
			<span>전체메뉴</span>
			<a href="javascript:;" class="float-right sidebar-close" title="닫기">
			<iconify-icon style="font-size:28px; color:#606060;" icon="jam:close"></iconify-icon>
	...
	<div  id="nt_sidebar_body" class="sidebar-content pb-5">
		...
		<!-- Icon -->
		<ul class="row row-cols-4 no-gutters f-sm text-center bg-white sidebar-icon mr-n1 sidefour">
		...
		<div class="mt-n1 sidelogin">
			<?php if($is_member) { // 로그인시 ?>
				<div class="btn-group w-100" role="group" aria-label="Member Menu">
					<a href="javascript:;" onclick="sidebar('user')" class="btn rounded-0 ham-a-but" role="button">
						내정보
					</a>
					<?php if(IS_NA_NOTI) { ?>
						<a href="javascript:;" onclick="sidebar('noti')" class="btn rounded-0 ham-a-but" role="button" title="알림">
					...
					<?php } ?>
					<a href="<?php echo G5_BBS_URL ?>/logout.php" class="btn ham-a-but" role="button">
						로그아웃
					</a>
				</div>
				<!-- User -->
			...
			<?php } else { ?>
				<div class="btn-group w-100" role="group" aria-label="Member Menu">
					<a href="<?php echo G5_BBS_URL ?>/login.php?url=<?php echo $urlencode ?>" class="btn ham-a-but" onclick="sidebar('login'); return false;" role="button">
						로그인
					</a>
					<a href="<?php echo G5_BBS_URL ?>/register.php" class="btn win ham-a-but" role="button">
						회원가입
					</a>
				</div>
				<!-- Login -->
